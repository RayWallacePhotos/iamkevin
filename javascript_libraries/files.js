...
// fileFileOfImageTextFiles( fileName, {basePath, callback} )
// fileFileOfImageFiles( fileName, {basePath, callback} )
// loadNextImage( next, max, fileList, callback )
// fileFileOfTextFiles( fileName, {basePath, callback} )
...
//
// Specified fileName contains a list of fileNames of matching image files
//
// Return an object to the callback, with the image, {name:fileName, image:imageElement}
//
// NOTE: If either file could not be read, it's respective value (image or text) will be null
//
function fileFileOfImageFiles( fileName, {basePath="./", textFile=false, callback=null}={} ) {
  fileFileOfImageTextFiles( fileName, {basePath, textFile:false, callback} );
}
...
// NOTE: If either file could not be read, it's respective value (image or text) will be null
//
function fileFileOfImageTextFiles( fileName, {basePath="./", textFile=true, callback=null}={} ) {
  let next;
  let fileNames = [];
  let nextFile;
  ...
  fileReadText( fileName, (textObj) => {
    ...
    if( textObj.text ) {
      ...
      loadNextImage( 0, fileNames, {basePath, textFile, callback} );
    }
    else callback( {name:null, image:null, text:null} ); // Indicate we didn't/couldn't read anything
  } );
}
...
//
function loadNextImage( next, fileList, {basePath, textFile, callback} ) {
  if( next < fileList.length ) {
    // First load the image from the filelist
    fileLoadImage( basePath + fileList[next], imageObj => {
      // imageObj might be null if image file could not be loaded (not found etc...)
      if( imageObj ) {
        if( !textFile ) {
          // Finally, return both the imageElement
          callback( {name:imageObj.fileName, image:imageObj.element, text:null} );

          // Recursion until we're done
          loadNextImage( ++next, fileList, {basePath, textFile, callback} );
        }
        else {
          // Then replace the .ext and read the text file with the same name as the image file
          fileReadText( changeExtension( basePath + imageObj.fileName, ".txt" ), textObj => {
            // Finally, return both the imageElement and the text (if there was a text file)
            callback( {name:imageObj.fileName, image:imageObj.element, text:textObj.text} );
        ...
        callback( {name:null, image:null, text:null} ); // Indicate we didn't/couldn't read anything
...
//
function fileFileOfTextFiles( fileName, {basePath="", callback=null}={} ) {
  let next;
  let fileNames = [];
  ...
  fileReadText( fileName, (textObj) => {
    ...
    if( textObj.text ) {
      ...
      loadNextFile( 0, fileNames, {basePath, callback} );
    }
...
console.log(`DEBUG loadNextFile: ${basePath + fileList[next]}`);
      ...
      // Recursion until we're done
      loadNextFile( ++next, fileList, {basePath, callback} );
    } );
  }
}
...
//      if(response.ok) textStream = response.text(); // .status is in the range 200-299
    ...
    .then( text => {
      // .decode(text) to Deal with some funky characters cut/pasted from MS Word docs (smart quotes etc..)
